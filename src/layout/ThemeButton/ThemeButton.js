...
const ThemeButton = () => {
  ...
  const themeColorButtons = colorTheme.map((color) => {
    let arr = color.split("");
    let firstElement = arr.shift().toUpperCase();
    let capitalize = [firstElement, ...arr];
    let rightClass = `color${capitalize.join("")}`;
    ...
    return (
      <ThemeColorButton
        ...
        nameOfClass={rightClass}
        switchTheme={handlethemeActive}
      />
    );
  });
    ...
    <>
      <div className={`${style.themeButtons} flex-column`} ref={themeButtonRef}>
        <svg
          ...
          fill="currentColor"
          className={`${style.bucketBtn} bi bi-paint-bucket`}
          viewBox="0 0 16 16">
